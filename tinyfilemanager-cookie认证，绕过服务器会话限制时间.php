...
if ($use_auth) {
    ...
    //第二次认证时，有cookie就直接登录，无须经过session。
    if(isset($_COOKIE['envlpass'])  &&  password_verify($_COOKIE['envlpass'], $auth_users['admin'])){
        // Logged
        ...
        if (function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']]) && verifyToken($_POST['token'])) {
                ...
                //第一次认证过后，session容易过时，绕过sesion认证，转用cookie
                setcookie('envlpass',($_POST['fm_pwd']));
        ...
        //退出时把cookie清空
        setcookie('envlpass',null);
        // fm_set_msg('没有session id', 'error');;
        $has_session = isset($_SESSION[FM_SESSION_ID]['logged']);
        $session_value = $has_session ? $_SESSION[FM_SESSION_ID]['logged'] : '未设置';

        $is_authorized = $has_session && isset($auth_users[$_SESSION[FM_SESSION_ID]['logged']]);
        $auth_status = $is_authorized ? '已授权' : '未授权';

        // 合并两条信息，用换行或分隔符隔开
        $combined_message = "Session 状态: " . ($has_session ? "存在 (用户: $session_value)" : "不存在") . "\n" 
                        . "授权状态: " . $auth_status;

        fm_set_msg($combined_message, $is_authorized ? 'success' : 'error');
?>
        <section class="h-100">
